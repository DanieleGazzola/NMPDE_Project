...
void Poisson3DParallel::generateSphere(const unsigned int n, const double r)
{
  radius = r;
  centres.reserve(n);
  for(std::size_t i = 0; i < n; ++i)
  {
    ...
    for(std::size_t j = 0; j < dim; ++j)
    {
      ...
      p[j] = unitary(engine);
      pcout << p[j] << std::endl;
      while(p[j] + radius > 1 || p[j] - radius < 0)
      {
        p[j] = unitary(engine);
        pcout << p[j] << std::endl;
      }     
    }
    if(!sphereIsOverlapped(p)){
      centres.emplace_back(p);
    }
    else --i;
  }
}

bool Poisson3DParallel::sphereIsOverlapped(const Point<dim> &p)
{
  for(std::size_t i = 0; i < centres.size(); ++i)
  {
    double sum = (p[0] - centres[i][0]) * (p[0] - centres[i][0]) + (p[1] - centres[i][1]) * (p[1] - centres[i][1]) + (p[2] - centres[i][2]) * (p[2] - centres[i][2]);
    if (sum < 2*radius * radius)
      return true;
  }
  return false;
}

bool Poisson3DParallel::pointIsInASphere(const Point<dim> &p)
{
  for(std::size_t i = 0; i < centres.size(); ++i)
  {
    double sum = (p[0] - centres[i][0]) * (p[0] - centres[i][0]) + (p[1] - centres[i][1]) * (p[1] - centres[i][1]) + (p[2] - centres[i][2]) * (p[2] - centres[i][2]);
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  if(!pointIsInASphere(fe_values.quadrature_point(q)))
                  {
                  ...
                  {
                    cell_matrix(i, j) += diffusion_coefficient_inside.value(
                                         fe_values.quadrature_point(q)) // mu(x)
                                       * fe_values.shape_grad(i, q)     // (I)
                                       * fe_values.shape_grad(j, q)     // (II)
                                       * fe_values.JxW(q);              // (III)   
...
Poisson3DParallel::output() const
{
  ...
  // Finally, we need to write in a format that supports parallel output. The
  // combination of H5 and XDMF is one possible options (which is advantageous
